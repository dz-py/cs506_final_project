...
try:
    script_dir = os.path.dirname(os.path.abspath(__file__)) # Path of the script file
except NameError:
    # __file__ not defined (e.g., running in interactive mode like Jupyter)
    script_dir = os.getcwd()
    logging.warning(f"__file__ not found, using current working directory: {script_dir}")

project_root = os.path.dirname(script_dir) # Go up one level (assumes script in 'scripts' folder)
...
train_datagen = ImageDataGenerator(
    ...
    channel_shift_range=20.0  # Random channel shifts for color augmentation
)
# Validation data uses only preprocessing, no augmentation
...
# Find the ResNet base model layer by its explicit name
base_model_layer = None
target_base_model_name = "resnet152_base" # Name assigned in build_food_classifier

for layer in model.layers:
    # Look for the layer with the correct name and ensure it's a Keras Layer
...
# If not found, raise an error with available layer names for debugging
...
# =========================
# Set the base model to trainable so we can fine-tune its weights
base_model_layer.trainable = True
logging.info(f"Base model layer '{base_model_layer.name}' set to trainable.")

# Freeze all layers within the base model except the last 30 for fine-tuning
    ...
    if layers_to_freeze > 0:
        ...
        for i, internal_layer in enumerate(base_model_layer.layers):
            if i < layers_to_freeze:
                ...
                internal_layer.trainable = True # Last 30 layers are trainable
    else:
...
optimizer_finetune = tf.keras.optimizers.Adam(learning_rate=1e-5) # Smaller LR for fine-tuning
...
csv_logger_finetune = CSVLogger(csv_logger_path, append=True) # Continue logging in the same file

# Determine which epoch to start fine-tuning from (accounts for early stopping)
if history.epoch:
...
finetune_history = model.fit(
    train_gen,
    validation_data=val_gen,
    epochs=initial_fine_tune_epoch + fine_tune_epochs, # Continue from last epoch
...
# =========================
# Merge initial and fine-tuning histories for later analysis/plotting
...
if hasattr(finetune_history, 'history'):
    for key in finetune_history.history:
        if key in combined_history:
            ...
            # For keys only present in fine-tuning, pad with None for initial epochs
