...
def build_food_classifier(input_shape=(224, 224, 3), num_classes=6):
    ...
    resnet_base = ResNet152(weights='imagenet', include_top=False, input_shape=input_shape)
...
checkpoint_path = os.path.join(model_dir, 'best_food_classifier.h5')  # Changed from .keras to .h5
checkpoint = ModelCheckpoint(
    checkpoint_path,
    monitor='val_accuracy',
    save_best_only=True,
    verbose=1,
    save_format='h5'  # Explicitly specify save format
)
...
target_base_model_name = "resnet152"  # Changed from 'resnet152_base' to 'resnet152'
...
final_model_path = os.path.join(model_dir, "food_classifier_final.h5")  # Changed from .keras to .h5
try:
    model.save(final_model_path, save_format='h5')  # Explicitly specify save format
