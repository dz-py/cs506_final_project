...
# Configuration
# =========================
NUM_MAIN_CLASSES = 5  # Number of main food classes to select
NUM_IMAGES_PER_MAIN_CLASS = 50 # Number of images per main class
NUM_IMAGES_PER_REMAINING_CLASS = 5 # Number of images per *original* remaining class to put into 'unknown'
...
OUTPUT_DIR_NAME = "food-101-filtered-5main-unknown" # Name for the output directory in project root
# =========================
...
try:
    ...
    # A simple check: if 'scripts' is in the cwd path, assume we need the parent.
...
# Define the path where the filtered images will be saved - **MODIFIED** to be in project root
filtered_images_dir = os.path.join(project_root, OUTPUT_DIR_NAME)
...
if os.path.exists(filtered_images_dir):
    ...
    try:
        ...
        logging.error(f"Error removing directory {filtered_images_dir}: {e}")
        exit(1)
try:
    os.makedirs(filtered_images_dir, exist_ok=True)
    logging.info(f"Created empty filtered directory: {filtered_images_dir}")
except OSError as e:
     logging.error(f"Error creating directory {filtered_images_dir}: {e}")
     exit(1)
...
remaining_classes = sorted(list(set(all_classes) - set(main_classes)))
logging.info(f"Identified {len(remaining_classes)} remaining classes for the '{UNKNOWN_CLASS_DIR_NAME}' category.")
...
# =========================
logging.info(f"Copying {NUM_IMAGES_PER_REMAINING_CLASS} images from each of the {len(remaining_classes)} remaining classes into '{UNKNOWN_CLASS_DIR_NAME}'...")
...
for class_name in remaining_classes:
    ...
    try:
        ...
        num_to_select = min(num_available, NUM_IMAGES_PER_REMAINING_CLASS) # Select fewer if not enough

        if num_available < NUM_IMAGES_PER_REMAINING_CLASS:
            logging.warning(f"Class '{class_name}' only has {num_available} images, requested {NUM_IMAGES_PER_REMAINING_CLASS} for unknown. Selecting all available.")
